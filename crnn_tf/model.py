...
class Model(object):
  ...
  def convRelu(self, i, is_training=False, batch_norm=False):
    ...
    self.net = tf.layers.conv2d(self.net, self.nm[i], [self.ks[i], self.ks[i]], padding=ps, activation=None, name='Conv2d_%d_%dx%d'%(i, self.ks[i], self.ks[i]))
    if batch_norm:
  ...
  def _build_training(self):
    ...
    self.batch_size, self.height, self.width = input_shape[0], input_shape[1], input_shape[2]
    self.net = tf.reshape(self.images, [self.batch_size, self.height, self.width, 1])
    ...
    # conv1-7
    self.convRelu(1)
    self.net = tf.layers.max_pooling2d(self.net, pool_size=[2, 2], strides=[2, 2], name='pool1')    # b*16*50*64

    self.convRelu(2)
    self.net = tf.layers.max_pooling2d(self.net, pool_size=[2, 2], strides=[2, 2], name='pool2')    # b*8*25*128

    self.convRelu(3, is_training=self.is_training, batch_norm=True)
    self.convRelu(4)                                                                                 # b*8*25*256
    self.net = tf.pad(self.net, paddings=[[0, 0], [0, 0], [1, 1], [0, 0]])                           # b*8*27*256
    self.net = tf.layers.max_pooling2d(self.net, pool_size=[2, 2], strides=[2, 1], padding='VALID', name='pool3')    # b*4*26*256

    self.convRelu(5, is_training=self.is_training, batch_norm=True)
    self.convRelu(6)
    self.net = tf.pad(self.net, paddings=[[0, 0], [0, 0], [1, 1], [0, 0]])                           # b*8*27*256
    self.net = tf.layers.max_pooling2d(self.net, pool_size=[2, 2], strides=[2, 1], padding='VALID', name='pool4')    # b*2*27*512
    ...
    self.decoded, self.log_prob = tf.nn.ctc_greedy_decoder(logits, self.seqs_length)

    # Inaccuracy: label error rate
    self.ler = tf.reduce_mean(tf.edit_distance(tf.cast(self.decoded[0], tf.int32), self.targets), name='ler')


  def train_minibatch(self, session, images, is_training, seqs_length, targets):
  ...
  def set_learning_rate(self, to_lr=None):
    if not to_lr is None and to_lr != self.lr:
      print 'Changing lr to %f...'%to_lr
      self.lr = to_lr
