...
import sys
sys.path.insert(0, '/home/liuhu/workspace/ctc_ocr')
    ...
    for epoch in range(config.items['starting'], config.items['max_epoch']):
        ...
        if WER_valid < min_wer[0]:
            ...
            if patienced_epoch > config.items['patience']:
                patienced_epoch = 0
                model.set_learning_rate(np.array(model.lr * 0.2, dtype=np.float32))
        ...
        # for iter, train_inputs in enumerate(train_set.iterate()):
            ...
            train_return = model.train_minibatch(sess, images, True, mask.sum(axis=1), targets)
            # pdb.set_trace()
            ...
            if iter % config.items['disp_iter']==0:
                glog.info('Phase = %s, Epoch %d, Iteration %d, lr = %.2e, Training loss = %s' %
                          (phase, epoch, iter, model.lr, np.mean(losses, axis=0) if len(losses.shape)>1 else losses))
